...
function Strategy(options, verify) {
  ...
  this._tokenName = options.tokenName || 'access_token';
  if (options.scope) {
...
Strategy.prototype.authenticate = function(req) {
  ...
  if (req.body && req.body[this._tokenName]) {
    if (token) { return this.fail(400); }
    token = req.body[this._tokenName];
  }

  if (req.query && req.query[this._tokenName]) {
    if (token) { return this.fail(400); }
    token = req.query[this._tokenName];
  }
